...
from src.models.players.agent import LLMAgent
...
class ResistanceCoupGameHandler:
    ...
    def __init__(self, player_name: str, number_of_players: int):
        ...
        for i in range(number_of_players - 1):
            ...
            self._players.append(AIPlayer(name=ai_name, game=self))


        self._players.append(LLMAgent(name='LLMAgent', game=self))
    ...
    def _action_phase(
        ...
        target_action, target_player = self.current_player.choose_action(players_without_current, self._revealed_cards, len(self._deck), int(self._treasury))
    ...
    def _challenge_against_player_failed(
        ...
        self._revealed_cards.append(challenger.remove_card(self))
    ...
    def _challenge_against_player_succeeded(self, player_being_challenged: BasePlayer):
        ...
        self._revealed_cards.append(player_being_challenged.remove_card(self))
    ...
    def _challenge_phase(
        ...
        for challenger in other_players:
            should_challenge = challenger.determine_challenge(player_being_challenged, self)
            if should_challenge:
                if challenger.is_ai:
    ...
    def _counter_phase(
        ...
        for countering_player in players_without_current:
            should_counter = countering_player.determine_counter(self.current_player, self)

            if should_counter:
                target_counter = get_counter_action(target_action.action_type)
                
                countering_player.last_action = target_counter
                countering_player.last_target = self.current_player
                print_text(
                    build_counter_report_string(
        ...
        match action.action_type:
            ...
            case ActionType.coup:
                ...
                if target_player.cards:
                    ...
                    self._revealed_cards.append(target_player.remove_card(self))
            case ActionType.tax:
                # Player gets 3 coins
            ...
            case ActionType.exchange:
                ...
                first_card, second_card = self.current_player.choose_exchange_cards(cards,self)
                self._deck.append(first_card)
                self._deck.append(second_card)

    def handle_turn(self) -> bool:
        # for player in self._players:
            # player.choose_exchange_cards([self._deck[0], self._deck[1]],self)
        # exit()
        ...
        self.current_player.last_action = target_action
        self.current_player.last_target = target_player
        ...
        if self._determine_win_state():
            ...
            if self.remaining_player.is_agent:
                return True, True
            else:
                return True, False
