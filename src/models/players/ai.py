...
class AIPlayer(BasePlayer):
    ...
    def choose_action(self, other_players: List[BasePlayer],revealed_cards:List[Card], 
                  deck_len:int, treasury:int) -> Tuple[Action, Optional[BasePlayer]]:
    ...
    def determine_challenge(self, player: BasePlayer, game) -> bool:
        ...
        # return agent_determine_challenge(player, self, game)
...
class AIPlayerV0(AIPlayer):
    def choose_action(self, other_players: List[BasePlayer], 
                      burned_cards:List[Card], 
                      deck_len:int, treasury:int) -> Tuple[Action, BasePlayer | None]:
        # new implementation of choose action 
        available_actions = self.available_actions()
        print_text(f"[bold magenta]{self}[/] is thinking...", with_markup=True)
        # Coup is only option
        if len(available_actions) == 1:
            # TODO player needs help to choose the right player to coup
            player = random.choice(other_players)
            return available_actions[0], player
        # TODO Pick any other choice (might be a bluff)
        # choose targe player and target action at the same time
        target_action = random.choice(available_actions)
        target_player = None
